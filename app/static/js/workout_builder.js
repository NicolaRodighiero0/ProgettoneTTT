   ...
   document.addEventListener('DOMContentLoaded', () => {
    ...
    const makeExerciseSelect = (selected = null) => {
      ...
      exerciseOptions.forEach(ex => {
        sel.innerHTML += `<option value="${ex.id}" ${ex.id==selected?'selected':''}>
                            ${ex.name}</option>`;
      });
      return sel;
    };
    ...
    const makeMachineSelect = (selected = null) => {
      ...
      machineOptions.forEach(m => {
        sel.innerHTML += `<option value="${m.id}" ${m.id==selected?'selected':''}>
                            ${m.name}</option>`;
      });
      return sel;
    };
    ...
    const applyFlags = (tr, ex) => {
      const toggle = (cls, on) =>
        tr.querySelector(cls).disabled = !on;
    ...
    const addRow = (prefill = {}) => {
      ...
      /* select macchinario */
      tr.children[2].appendChild(makeMachineSelect(prefill.machine_id));
    ...
    /* se ci sono esercizi, crea la prima riga all’apertura */
    if (exerciseOptions && exerciseOptions.length && !tableBody.children.length) {
      addRow();
    }
  });
